...
MESH_PATH = PROJECT_ROOT / "data" / "cache" / "final_test" / "airfoil.su2"
...
if history_path.exists():
    ...
    last_data = None
    for line in reversed(hist_lines[1:]):
        s = line.strip()
        if s and s != ',':
            last_data = s
            break
    ...
    if last_data:
        ...
        if 0.3 <= cl <= 0.7:
            ...
            print(f"  [WARN] CL={cl:.4f} outside expected range [0.3, 0.7]")

        if 0.008 <= cd <= 0.025:
            print(f"  [OK] CD={cd:.4f} within expected range [0.008, 0.025]")
        else:
            print(f"  [WARN] CD={cd:.4f} outside expected range [0.008, 0.025]")
