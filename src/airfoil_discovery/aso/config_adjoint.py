...
def generate_adjoint_config(
    ...
    lines = [
        f"% ------- SU2 Continuous Adjoint Configuration -------",
        f"% Generated by airfoil_discovery.aso.config_adjoint",
        f"% Objective: {objective}",
        f"",
        f"% ------------ Solver ------------",
        f"SOLVER= {solver}",
        f"MATH_PROBLEM= CONTINUOUS_ADJOINT",
        f"RESTART_SOL= YES",
        f"",
    ...
    # For continuous adjoint, SU2 expects the primal restart file with a specific naming convention
    # The pattern is: solution_adj_<objective>.dat where objective is cd, cl, etc.
    if restart_filename:
        from pathlib import Path as _Path
        restart_path_obj = _Path(restart_filename)
        if restart_path_obj.exists():
            # Copy/rename the primal restart to the expected adjoint filename
            ...
            expected_name = f"solution_adj_{obj_to_filename.get(obj_value, obj_value.lower())}.dat"
            lines.append(f"SOLUTION_FILENAME= {expected_name}")
